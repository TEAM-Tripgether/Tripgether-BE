-- ===================================================================
-- 미디어 구조 간소화 마이그레이션 (기존 테이블 안전 대응)
-- Version: 0.1.5
...
-- ===================================================================
-- PART 1: 의존 테이블 스키마 보정 (기존 테이블이 있어도 안전)
-- ===================================================================

-- 1-1. Member 테이블 스키마 보정
DO $$
BEGIN
    -- 테이블이 없으면 생성
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'member') THEN
        CREATE TABLE member (
            id UUID PRIMARY KEY,
    ...
    ELSE
        -- 기존 테이블에 누락된 컬럼 추가
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'member' AND column_name = 'name') THEN
            ALTER TABLE member ADD COLUMN name VARCHAR(100);
            -- 기존 데이터 처리: email 앞부분을 name으로 사용
            UPDATE member SET name = SPLIT_PART(email, '@', 1) WHERE name IS NULL;
            ALTER TABLE member ALTER COLUMN name SET NOT NULL;
        END IF;
        
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'member' AND column_name = 'profile_image_url') THEN
            ALTER TABLE member ADD COLUMN profile_image_url TEXT;
        END IF;
    END IF;
END $$;

-- Member 테이블 인덱스 생성 (컬럼 확보 후)
...
-- 1-2. Place 테이블 스키마 보정
DO $$
BEGIN
    -- 테이블이 없으면 생성
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'place') THEN
        CREATE TABLE place (
            id UUID PRIMARY KEY,
    ...
    -- 기존 테이블이 있으면 필요한 컬럼만 추가 (현재 Place는 모두 베이스라인에 포함됨)
    END IF;
END $$;
...
BEGIN
    ...
    -- 기존 테이블이 있으면 필요한 컬럼만 추가 (현재 Content는 모두 베이스라인에 포함됨)
    END IF;
END $$;
...
-- 1-4. Folder 테이블 스키마 보정
DO $$
BEGIN
    -- 테이블이 없으면 생성
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'folder') THEN
        CREATE TABLE folder (
            id UUID PRIMARY KEY,
    ...
    ELSE
        -- 기존 테이블에 thumbnail_url 컬럼 추가 (V0.1.5의 주요 목적)
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'folder' AND column_name = 'thumbnail_url') THEN
...
-- 1-5. Folder 외래키 제약조건 추가 (member 테이블 존재 보장 후)
DO $$
BEGIN
    IF NOT EXISTS (
...
-- 2-1. place_media 테이블 생성/수정
DO $$
BEGIN
    -- 테이블이 없으면 생성
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'place_media') THEN
    ...
    ELSE
        -- 기존 테이블에 url 컬럼 추가 (media_id 방식에서 url 방식으로 전환)
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'place_media' AND column_name = 'url') THEN
            ALTER TABLE place_media ADD COLUMN url TEXT;
        END IF;
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'place_media' AND column_name = 'mime_type') THEN
            ALTER TABLE place_media ADD COLUMN mime_type VARCHAR(100);
        END IF;
    END IF;
END $$;
...
-- 3-1. content_media 테이블 생성/수정
DO $$
BEGIN
    -- 테이블이 없으면 생성
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'content_media') THEN
    ...
    ELSE
        -- 기존 테이블에 url 컬럼 추가 (media_id 방식에서 url 방식으로 전환)
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'content_media' AND column_name = 'url') THEN
            ALTER TABLE content_media ADD COLUMN url TEXT;
        END IF;
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'content_media' AND column_name = 'mime_type') THEN
            ALTER TABLE content_media ADD COLUMN mime_type VARCHAR(100);
        END IF;
    END IF;
END $$;
...
-- ===================================================================
-- PART 4: member 테이블에 profile_image_url 추가
-- ===================================================================

-- ✅ 이미 PART 1-1에서 처리됨 (중복 제거)

-- ===================================================================
-- PART 5: folder 테이블에 thumbnail_url 추가
-- ===================================================================

-- ✅ 이미 PART 1-4에서 처리됨 (중복 제거)
