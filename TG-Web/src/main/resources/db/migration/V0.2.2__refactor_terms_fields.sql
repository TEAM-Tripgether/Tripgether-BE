...
-- 2. 기존 데이터 마이그레이션 (requiredAgreed가 true면 true로 설정)
UPDATE public.member
SET is_service_terms_and_privacy_agreed = COALESCE(required_agreed, false)
WHERE is_service_terms_and_privacy_agreed IS NULL;

-- 3. 기존 회원 약관 동의 완료 처리 (null인 경우 true로 설정)
UPDATE public.member
